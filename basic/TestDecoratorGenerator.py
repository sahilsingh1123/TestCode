...
def decoratorFuncTwo(func):
    ...
    def wrapperFunc(*args, **kwargs):
        print(
            "from Method (2) execute these above method when below func is used with decorator {}.".format(
                func.__name__
            )
        )
        return func(*args, **kwargs)
...
class decoratorClass(object):
    ...
    def __call__(self, *args, **kwargs):
        print(
            "from class execute these above method when below func is used with decorator {}.".format(
                self.func.__name__
            )
        )
